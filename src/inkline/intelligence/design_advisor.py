...
class DesignAdvisor:
    ...
    def _build_plan_system_prompt(self) -> str:
        ...
        return "\n".join([
            ...
            "",
            "FORBIDDEN AT PLANNING TIME (text/number-only — no visual evidence):",
            "  stat, kpi_strip, icon_stat   — bare number slides with no chart context",
            ...
            "",
            "  timeline      — horizontal milestone strip (3-6 events with dates).",
            "  process_flow  — numbered steps with connecting arrows (3-5 steps).",
            ...
            "  Sequential events / milestones           → timeline",
            "  Process / workflow                       → process_flow",
            "  Qualitative narrative with NO data       → chart_caption (use an iceberg/ladder",
            "                                             structural chart to visualise the point)",
            "",
            "═══════════════════════════════════════════════════════════════",
            "PLANNING RULES",
            "═══════════════════════════════════════════════════════════════",
            "- Start title (source_index=0), end closing (source_index=0)",
            "- section_divider: use sparingly — only for major theme breaks (max 5 for a 40-slide deck)",
    ...
    def _review_plan_llm(
        ...
        *,
        ...
        user_prompt = "\n".join([
            ...
            "   stat, kpi_strip, icon_stat are BANNED — they show numbers without context.",
