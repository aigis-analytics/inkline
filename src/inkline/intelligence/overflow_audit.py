...
def audit_deck_with_llm(
    ...
    *,
    ...
    # Build an explicit (slide_index, page_png) mapping that accounts for
    # overflow pages.  When a slide overflows onto an extra page that page
    # must be skipped so subsequent slides are not audited against the wrong
    # slide data.  overflow_slide_indices contains 0-based slide indices that
    # are known to produce extra pages (exactly 1 extra page per entry assumed).
    overflow_set = set(overflow_slide_indices or [])
    slide_page_pairs: list[tuple[int, Path]] = []  # (slide_idx, page_png)
    page_i = 0
    for slide_i in range(len(slides)):
        if page_i >= len(page_pngs):
            break
        slide_page_pairs.append((slide_i, page_pngs[page_i]))
        page_i += 1
        if slide_i in overflow_set:
            page_i += 1  # skip the one overflow page generated by this slide

    n_pairs = len(slide_page_pairs)

    # Audit slides in parallel — each slide is an independent vision call.
    from concurrent.futures import ThreadPoolExecutor, as_completed

    def _audit_one(slide_i: int, png: Path) -> tuple[int, list[AuditWarning]]:
        slide_type = slides[slide_i].get("slide_type", "?")
        slide_data = slides[slide_i].get("data", {})
        slide_source = slide_data.get("source_section", "") or source_narrative
        page_warnings = audit_slide_with_llm(
            png,
            slide_index=slide_i + 1,
            slide_type=slide_type,
    ...
    warnings: list[AuditWarning] = []
    for slide_i, _ in slide_page_pairs:
        warnings.extend(results.get(slide_i, []))
