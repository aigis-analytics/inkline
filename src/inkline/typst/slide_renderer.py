...
#set par(leading: 0.7em)"""
  ...
  // Top accent stripe — full bleed
  #place(top + left, dx: -1.6cm, dy: -1.4cm, block(width: 25.4cm, height: 6pt, fill: {_rgb(t['accent'])}))

  // Top-right "GENERATED BY INKLINE" tag
  #place(top + right, dx: 1.6cm, dy: -1.4cm + 6pt,
    block(width: 5.4cm, height: 1.0cm, fill: {_rgb(t['accent'])})[
      #align(center + horizon)[#text(size: 8pt, weight: "bold", tracking: 2pt, fill: white)[GENERATED BY INKLINE]]
    ]
  )
  ...
  #text(weight: "bold", size: 80pt, font: "{heading_font}", tracking: -2pt)[{_esc(company)}]

  #v(0.2cm)

  // Tagline (smaller when subtitle is also present)
  #block(width: 19cm)[
    #text(weight: "bold", size: {18 if subtitle else 22}pt, font: "{heading_font}", fill: {_rgb(t['title_fg'])})[{_esc(tagline)}]
  ]

  {f'#v(0.3cm){chr(10)}  #block(width: 19cm)[#text(size: 12pt, fill: {_rgb(t["muted"])})[{_esc(subtitle)}]]' if subtitle else ''}

  // Bottom row pinned to absolute bottom via place() — cannot overflow
  #place(bottom + left, dx: 0cm, dy: 0cm,
    block(width: 22.2cm)[
      #grid(
        ...
        align: bottom,
        column-gutter: 14pt,
        block(width: 1cm, height: 1cm, fill: {_rgb(t['accent'])}),
        block[
          #text(weight: "bold", size: 11pt, fill: {_rgb(t['title_fg'])})[{_esc(left_footer)}]
          #v(2pt)
          #text(size: 9pt, fill: {_rgb(t['muted'])})[{_esc(date)}]
        ],
        align(right + bottom)[
          #text(size: 8pt, weight: "bold", tracking: 1pt, fill: {_rgb(t['muted'])})[OPEN SOURCE · MIT LICENSED]
        ],
      )
    ]
  )
  ...
  // Top-right "GENERATED BY INKLINE" tag (matches title slide)
  #place(top + right, dx: 1.6cm, dy: -1.4cm + 6pt,
    block(width: 5.4cm, height: 1.0cm, fill: {_rgb(t['accent'])})[
      #align(center + horizon)[#text(size: 8pt, weight: "bold", tracking: 2pt, fill: white)[GENERATED BY INKLINE]]
    ]
  ...
  #v(1.6cm)
  #align(center)[
    #text(weight: "bold", size: 56pt, font: "{heading_font}", tracking: -1pt)[{_esc(company)}]
    #v(0.3cm)
  ...
  // Bottom accent square + caption (matches title slide chrome)
  #grid(
    columns: (auto, 1fr, auto),
    align: bottom,
    column-gutter: 14pt,
    ...
    align(right + bottom)[
      #text(size: 8pt, weight: "bold", tracking: 1pt, fill: {_rgb(t['muted'])})[github.com/u3126117/inkline]
    ],
  )
]"""
