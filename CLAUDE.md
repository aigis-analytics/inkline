...
for i, page in enumerate(doc):
    ...
    audit_prompt = (
        f"You are a senior design director at a top-tier investment bank. "
        f"Audit slide {i+1}/{n} against best-in-class institutional presentation standards. "
        "Be strict — a slide that merely avoids errors is not a pass. It must actively earn a pass.\n\n"

        "── GATE CHECKS (structural — any failure is an automatic FAIL) ──────────────\n"
        "G1. TRUNCATION: Is any text cut off with '...' or clipped at the slide edge?\n"
        "G2. PLACEHOLDER: Does any chart/image area show 'Chart not available' or a grey placeholder box?\n"
        "G3. OVERFLOW: Does any element bleed beyond the slide boundary or overlap the footer bar?\n"
        "G4. BLANK WASTE: Is more than 35% of the content area completely empty with no data, visuals, or intentional whitespace?\n\n"

        "── DESIGN QUALITY CHECKS (judge against playbook standards) ─────────────────\n"
        "D1. ACTION TITLE: Does the slide title state a specific insight or conclusion — not just a topic?\n"
        "    PASS: 'APP fraud caught 48h before payment clears via trajectory detection'\n"
        "    FAIL: 'Fraud Detection' or 'Problem Overview' — descriptive labels with no analytical point.\n\n"
        "D2. EXHIBITS OVER TEXT: Does the slide use visual encoding (chart, card, diagram, icon-stat) to carry\n"
        "    the data — or does it dump the information into bullet-list prose instead?\n"
        "    FAIL if a slide could have been a chart_caption or feature_grid but is instead a wall of bullets.\n\n"
        "D3. INFORMATION DENSITY: Does every visual element earn its space by encoding data or structuring\n"
        "    narrative? Zero tolerance for decorative fills, large empty margins around tiny content, or\n"
        "    a single KPI stranded in the centre of a half-empty slide.\n\n"
        "D4. COLOR DISCIPLINE: Does the slide use 3 colors or fewer? Is color applied to encode meaning\n"
        "    (primary = hero data, secondary = supporting, muted = background) — not for decoration?\n"
        "    FAIL if icon badges are rainbow-colored, or card accents are all different hues.\n\n"
        "D5. SO-WHAT TEST: Would a busy executive reading only this slide's title and visual understand\n"
        "    what action or conclusion is warranted? If the answer is 'I'd need to ask follow-up questions\n"
        "    to understand the point,' the slide has failed its job.\n\n"
        "D6. VISUAL HIERARCHY: Is there a clear primary focal point (the key number, the biggest bar, the\n"
        "    highlighted card) that the eye reaches first? Or does everything compete equally for attention?\n\n"
        "D7. ANTI-PATTERNS: Does the slide contain any of these known failures?\n"
        "    — Wall of bullets (6+ bullets with no visual structure)\n"
        "    — Numbers without context (a stat card with no trend, comparator, or supporting chart)\n"
        "    — Chart with axis labels so small they are illegible at normal viewing distance\n"
        "    — Commentary that repeats the chart data verbatim instead of interpreting it\n\n"

        "── VERDICT FORMAT ───────────────────────────────────────────────────────────\n"
        "Reply in exactly this format:\n"
        "PASS — [one sentence: what the slide does well]\n"
        "  or\n"
        "FAIL [Gx/Dx]: [specific violation] | FIX: [concrete action to resolve it]\n\n"
        "Multiple failures: list each on a new FAIL line. Be specific — name the element, not just the rule."
    )
